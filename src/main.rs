...
impl ShopifyClient {
    fn new() -> Result<Self, String> {
        let domain = std::env::var("SHOPIFY_STORE_DOMAIN").map_err(|_| "SHOPIFY_STORE_DOMAIN required")?;
        let token = std::env::var("SHOPIFY_ACCESS_TOKEN").map_err(|_| "SHOPIFY_ACCESS_TOKEN required")?;
        let clean = domain.trim_start_matches("https://").trim_start_matches("http://").trim_end_matches('/');
        Ok(Self {
    ...
    async fn get(&self, path: &str, params: &[(&str, &str)]) -> Result<Value, String> {
        ...
        let qs: Vec<String> = params.iter().filter(|(_, v)| !v.is_empty()).map(|(k, v)| format!("{k}={v}")).collect();
    ...
    async fn post(&self, path: &str, body: &Value) -> Result<Value, String> {
        ...
        if res.status().as_u16() == 204 { return Ok(json!({"ok": true})); }
        res.json().await.map_err(|e| format!("Parse: {e}"))
    }
...
fn n(v: &Value, k: &str) -> String { v[k].as_i64().map(|n| n.to_string()).unwrap_or_default() }
...
async fn call_tool_inner(name: &str, args: &Value, c: &ShopifyClient) -> Result<String, String> {
    match name {
        "list_products" => {
            ...
            let title = s(args,"title"); if !title.is_empty() { p.push(("title", &title)); }
            let vendor = s(args,"vendor"); if !vendor.is_empty() { p.push(("vendor", &vendor)); }
            let ptype = s(args,"product_type"); if !ptype.is_empty() { p.push(("product_type", &ptype)); }
            let cid = s(args,"collection_id"); if !cid.is_empty() { p.push(("collection_id", &cid)); }
            let status = s(args,"status"); if !status.is_empty() { p.push(("status", &status)); }
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "50".to_string() } else { lim }; p.push(("limit", &limit));
        ...
        "search_products" => {
            let q = s(args,"query");
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "50".to_string() } else { lim };
            ...
            if let Some(products) = data["products"].as_array() {
                ...
                let filtered: Vec<&Value> = products.iter().filter(|p| {
                    let txt = format!("{} {} {} {} {} {}", p["title"].as_str().unwrap_or(""), p["vendor"].as_str().unwrap_or(""), p["product_type"].as_str().unwrap_or(""), p["tags"].as_str().unwrap_or(""), p["handle"].as_str().unwrap_or(""), p["body_html"].as_str().unwrap_or(""));
                    txt.to_lowercase().contains(&ql)
                }).collect();
                Ok(serde_json::to_string_pretty(&json!({"products": filtered, "count": filtered.len()})).unwrap_or_default())
            } else { Ok(serde_json::to_string_pretty(&data).unwrap_or_default()) }
        }
        "product_count" => {
            let mut p: Vec<(&str,&str)> = Vec::new();
            let vendor = s(args,"vendor"); if !vendor.is_empty() { p.push(("vendor", &vendor)); }
        ...
        "list_collections" => {
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "50".to_string() } else { lim };
            let smart = c.get("/smart_collections.json", &[("limit", &limit)]).await?;
            let custom = c.get("/custom_collections.json", &[("limit", &limit)]).await?;
        ...
        "list_orders" => {
            let mut p: Vec<(&str,&str)> = Vec::new();
            let status = s(args,"status"); if !status.is_empty() { p.push(("status", &status)); } else { p.push(("status","any")); }
            let fs = s(args,"financial_status"); if !fs.is_empty() { p.push(("financial_status", &fs)); }
            let ffs = s(args,"fulfillment_status"); if !ffs.is_empty() { p.push(("fulfillment_status", &ffs)); }
            let cmin = s(args,"created_at_min"); if !cmin.is_empty() { p.push(("created_at_min", &cmin)); }
            let cmax = s(args,"created_at_max"); if !cmax.is_empty() { p.push(("created_at_max", &cmax)); }
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "50".to_string() } else { lim }; p.push(("limit", &limit));
        ...
        "recent_orders" => {
            let cnt = n(args,"count"); let count = if cnt.is_empty() { "10".to_string() } else { cnt };
            let data = c.get("/orders.json", &[("status","any"),("limit",&count),("order","created_at desc")]).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "unfulfilled_orders" => {
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "50".to_string() } else { lim };
            let data = c.get("/orders.json", &[("status","open"),("fulfillment_status","unfulfilled"),("limit",&limit)]).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "order_count" => {
            let mut p: Vec<(&str,&str)> = Vec::new();
        ...
        "search_customers" => {
            let q = s(args,"query");
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "25".to_string() } else { lim };
            let data = c.get("/customers/search.json", &[("query",&q),("limit",&limit)]).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "get_customer" => {
        ...
        "top_customers" => {
            let sort = s(args,"sort_by"); let sort_by = if sort.is_empty() { "total_spent".to_string() } else { sort };
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "20".to_string() } else { lim };
            let data = c.get("/customers.json", &[("limit",&limit),("order",&format!("{sort_by} desc"))]).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "check_inventory" => {
            ...
            if !iid.is_empty() {
                ...
                if !lid.is_empty() { p.push(("location_ids", &lid)); }
        ...
        "store_summary" => {
            let products = c.get("/products/count.json", &[("status","active")]).await?;
            let orders = c.get("/orders/count.json", &[("status","open")]).await?;
            Ok(serde_json::to_string_pretty(&json!({"active_products": products["count"], "open_orders": orders["count"]})).unwrap_or_default())
        }
        "sales_by_product" => {
            let cmin = s(args,"created_at_min");
            let cmax = s(args,"created_at_max");
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "250".to_string() } else { lim };
            let mut p = vec![("status","any"),("limit",&limit),("created_at_min",&cmin)];
            if !cmax.is_empty() { p.push(("created_at_max", &cmax)); }
            let data = c.get("/orders.json", &p).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "fulfillment_status_summary" => {
            ...
            for status in &["unfulfilled","partial","shipped"] {
                let data = c.get("/orders/count.json", &[("status","open"),("fulfillment_status",status)]).await?;
        ...
        "list_fulfillments" => {
            let id = s(args,"order_id");
            let data = c.get(&format!("/orders/{id}/fulfillments.json"), &[]).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "create_fulfillment" => {
            let id = s(args,"order_id");
            // Get fulfillment orders first
            let fo_data = c.get(&format!("/orders/{id}/fulfillment_orders.json"), &[]).await?;
            let fos = fo_data["fulfillment_orders"].as_array().ok_or("No fulfillment orders")?;
            let open: Vec<&Value> = fos.iter().filter(|f| f["status"].as_str() == Some("open")).collect();
            if open.is_empty() { return Err("No open fulfillment orders".into()); }
            ...
            if !tn.is_empty() {
                ...
                if !tc.is_empty() { ti["company"] = json!(tc); }
                if !tu.is_empty() { ti["url"] = json!(tu); }
                fulfillment["tracking_info"] = ti;
            }
            let data = c.post("/fulfillments.json", &json!({"fulfillment": fulfillment})).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "get_collection" => {
            ...
            match smart {
                ...
                Err(_) => {
                    let custom = c.get(&format!("/custom_collections/{id}.json"), &[]).await?;
        ...
        "collection_products" => {
            let id = s(args,"collection_id");
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "50".to_string() } else { lim };
            let data = c.get("/products.json", &[("collection_id",&id),("limit",&limit)]).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "create_smart_collection" => {
            ...
            if let Some(d) = args["disjunctive"].as_bool() { body["smart_collection"]["disjunctive"] = json!(d); }
            if let Some(p) = args["published"].as_bool() { body["smart_collection"]["published"] = json!(p); }
            let so = s(args,"sort_order"); if !so.is_empty() { body["smart_collection"]["sort_order"] = json!(so); }
            let bh = s(args,"body_html"); if !bh.is_empty() { body["smart_collection"]["body_html"] = json!(bh); }
        ...
        "create_discount_code" => {
            ...
            let data = c.post(&format!("/price_rules/{prid}/discount_codes.json"), &json!({"discount_code": {"code": code}})).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "list_discount_codes" => {
            let prid = s(args,"price_rule_id");
            let data = c.get(&format!("/price_rules/{prid}/discount_codes.json"), &[]).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "list_blogs" => {
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "25".to_string() } else { lim };
        ...
        "list_pages" => {
            ...
            if !ps.is_empty() { p.push(("published_status", &ps)); }
        ...
        "get_metafields" => {
            ...
            let mut p: Vec<(&str,&str)> = Vec::new();
            if !ns.is_empty() { p.push(("namespace", &ns)); }
        ...
        "set_metafield" => {
            ...
            let typ = s(args,"type");
            let data = c.post(&format!("/{rt}/{rid}/metafields.json"), &json!({"metafield": {"namespace": ns, "key": key, "value": val, "type": typ}})).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "list_webhooks" => {
            let lim = n(args,"limit"); let limit = if lim.is_empty() { "50".to_string() } else { lim };
            ...
            let mut p = vec![("limit", limit.as_str())];
            if !topic.is_empty() { p.push(("topic", &topic)); }
        ...
        "create_webhook" => {
            ...
            let format_val = if fmt.is_empty() { "json".to_string() } else { fmt };
            let data = c.post("/webhooks.json", &json!({"webhook": {"topic": topic, "address": address, "format": format_val}})).await?;
            Ok(serde_json::to_string_pretty(&data).unwrap_or_default())
        }
        "delete_webhook" => {
...
async fn main() {
    ...
    eprintln!("[shopify-mcp] Running (35 tools, base: {})", client.base_url);
    ...
    loop {
        ...
        let resp = match req.method.as_str() {
            "initialize" => json!({"jsonrpc":"2.0","id":req.id,"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{}},"serverInfo":{"name":"shopify-mcp","version":"1.0.0"}}}),
